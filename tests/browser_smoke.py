...
async def main():
    ...
    async with websockets.serve(relay,'127.0.0.1',0) as ws_server:
        ...
        async with async_playwright() as p:
            ...
            ok('Returning to cached timeline adds zero REQs',len(requests)==baseline+1)
            ...
            await second.wait_for_function("document.querySelector('#network-summary').textContent.includes('タブ間')")
            ok('Two tabs share exactly one relay WebSocket',len(connections)==1)
            ok('Second tab reuses persistent feed / metadata without REQ',len(requests)==before)
            await second.close()
