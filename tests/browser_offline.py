...
async def main():
    ...
    async with async_playwright() as p:
        ...
        ok('Known profile only fetches its posts; unopened list tabs make no request',await reqs()==3)
        await page.locator('[data-view="global"]').click();await page.wait_for_selector('.post')
        ok('Returning to global fetches fresh posts and reuses known session profiles',await reqs()==4)
